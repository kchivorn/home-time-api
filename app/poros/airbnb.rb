class Airbnb
  def initialize(data)
    raise ExceptionHandler::MissingReservationCode, 'Reservation code cannot be empty' unless data[:reservation_code]
    ...
    @status = data[:status]
    raise ExceptionHandler::MissingGuest, 'Guest cannot be empty' unless data[:guest]
